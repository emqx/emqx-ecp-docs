...
以系统/组织/项目管理员的身份登录 ECP，在告警页面的**告警规则配置**选项卡中，您也可以对告警规则进行设置。

![alert_rules](./_assets/alert-rules.png)

ECP 目前支持的告警规则包括 ECP 端通用规则、边缘服务触发的规则，及云端集群触发的规则三类。通用规则包括：邮件发送失败、Webhook 发送失败。由边缘服务触发的规则包括：驱动异常、规则异常、NeuronEX 离线、NeuronEX 重启。由云端集群触发的规则包括：EMQX 规则异常、EMQX 连接器异常。规则的详情请参见[统一运维服务 - 告警规则列表](../monitor/rules.md)。
