...
资源配置是基于 Kubernetes 安装部署的 ECP 初始化 Paas 层配置的模块，主要包括 [Kubernetes 连接设置](#配置-kubernetes-连接设置)、[镜像库](#配置镜像服务信息)、[配置存储类](#配置存储类)、[云端集群](#配置云端集群)和[边缘服务](#配置边缘服务)的初始化配置工作。在这个模块中，管理员可以通过添加或修改资源配置来满足应用需求。
...
#### NeuronEX Docker部署默认配置

可以通过边缘服务配置修改**NeuronEX**的默认配置值
