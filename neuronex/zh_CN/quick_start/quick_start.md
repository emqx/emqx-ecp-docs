...
本章节将指导用户从下载安装开始，以 Modbus TCP 驱动协议为例，快速开始使用 NeuronEX 采集模拟设备数据，并将数据直接上传到 MQTT Broker，或经过流处理的数据上传到云端 MQTT。
...
南向设备节点用于 NeuronEX 与设备建立连接、设备驱动协议的选择及设备数据采集点位的创建及配置。
...
详细的驱动地址使用说明请参阅 [模块配置](../configuration/south-devices/south-devices.md)。
:::
...
北向应用节点用于 NeuronEX 与多种北向应用进行数据交互，以 MQTT 插件为例，新增一个 MQTT 云连接模块。
...
NeuronEX 支持数据流处理，且自带一个 neuronStream 的数据流。本示例将介绍如何将 NeuronEX 采集到的数值进行 +1 操作后将结果发送到云端 MQTT 动态主题中。

### 新建数据流北向应用节点

新建一个数据流北向应用节点，操作步骤同上述**创建北向应用节点**，插件选择 **eKuiper**，同时在该创建的北向节点中订阅数据组。
